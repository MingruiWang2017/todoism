$(document).ready(function () {
    ...
    $(document).ajaxError(function (event, request) {
        ...
        if (request.responseJSON && request.responseJSON.hasOwnProperty('message')) {
            ...
            var IS_JSON = true;  //设置标志位，用来判断响应内容是不是json
            try {
    ...
    // 为POST、PUT、PATCH的ajax请求添加token header
    $.ajaxSetup({
    ...
    $(window).bind('hashchange', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#main').hide().html(data).fadeIn(800);  // 将响应内容渲染为局部页面到main元素中
                activeM();  // 激活新插入页面中的Materialize组件
            }
        });
    });
    ...
    function new_item(e) {
        ...
        if (e.which !== ENTER_KEY || !value) {
            return;  // 如果enter键没按下或输入为空，则不执行任何操作
        }
        $input.focus().val('');  // 聚焦到输入框并清空内容
        $.ajax({
            type: 'POST',
            url: new_item_url,
            data: JSON.stringify({'body': value}),  // 将数据化为json字符串
            contentType: 'application/json;charset=UTF-8', // POST请求如果不指定格式，默认是form格式
            success: function (data) {
                M.toast({html: data.message, classes: 'rounded'});
                $('.items').append(data.html);  // 将返回的条目HTML代码插入页面
                activeM();
                refresh_count();
            }
        });
    ...
    $(document).on('click', '.done-btn', function () {
        ...
        if ($item.data('done')) {
            $.ajax({
                type: 'PATCH',  // 使用PATCH请求更新部分数据
                url: $this.data('href'),
                success: function (data) {
                    $this.next().removeClass('inactive-item');
                    $this.next().addClass('active-item');  // done -> not done
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $this.next().addClass('inactive-item');  // not done -> done
